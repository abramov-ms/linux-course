...
struct intrusive_phonebook_user *search_user(const char *last_name)
{
	...
	while (node) {
		struct intrusive_phonebook_user *user = container_of(
			node, struct intrusive_phonebook_user, node);
		int ordering = strncmp(last_name, user->data.last_name,
				       PHONEBOOK_NAME_MAX);
		if (ordering < 0) {
			node = node->rb_left;
		} else if (ordering > 0) {
...
bool insert_user(struct intrusive_phonebook_user *user)
{
	...
	while (*new) {
		struct intrusive_phonebook_user *this = container_of(
			*new, struct intrusive_phonebook_user, node);
		int ordering = strncmp(user->data.last_name,
				       this->data.last_name,
				       PHONEBOOK_NAME_MAX);
		parent = *new;
		if (ordering < 0) {
			      ...
			      size_t size, loff_t *offset)
{
	...
	size_t errors = copy_to_user(buffer, &last_user->data,
			       ...
			       size_t size, loff_t *offset)
{
	...
	case phonebook_create: {
		...
		struct intrusive_phonebook_user *user = kmalloc(
			sizeof(struct intrusive_phonebook_user), GFP_KERNEL);
	     ...
	     struct phonebook_user __user *user)
{
	...
	char key[PHONEBOOK_NAME_MAX];
	size_t errors = copy_from_user(key, last_name, len);
	if (errors > 0) {
		status = -EINVAL;
		goto out;
	}

	struct intrusive_phonebook_user *value = search_user(key);
	if (!value) {
		status = -ENOENT;
		goto out;
	}
	...
	errors =
		copy_to_user(user, &value->data, sizeof(struct phonebook_user));
	if (errors > 0) {
		status = -EINVAL;
		goto out;
	}
...
int add_user(const struct phonebook_user __user *user)
{
	...
	struct intrusive_phonebook_user *container =
		kmalloc(sizeof(struct intrusive_phonebook_user), GFP_KERNEL);
	size_t errors = copy_from_user(&container->data, user,
				       sizeof(struct phonebook_user));
	if (errors > 0) {
		status = -EINVAL;
		goto out;
...
int del_user(const char __user *last_name, size_t len)
{
	...
	struct intrusive_phonebook_user *user = search_user(key);
	if (user == NULL) {
		status = -ENOENT;
		goto out;
	}
...
static struct phonebook book = { .get_user = &get_user,
				 .add_user = &add_user,
				 .del_user = &del_user };

static int __init phonebook_init(void)
{
	int status = register_phonebook(&book);
	if (status != 0) {
		return status;
	}
	printk(KERN_INFO "Registered phonebook syscalls");
...
static void __exit phonebook_exit(void)
{
	...
	class_destroy(cls);
	unregister_chrdev(dev_major, DEV_NAME);
	unregister_phonebook(&book);
	printk(KERN_INFO "Removed phonebook\n");
}
